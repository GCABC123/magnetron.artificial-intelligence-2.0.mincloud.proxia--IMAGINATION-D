...
ARTIFICIAL INTELLIGENCE 2.0 ™ : IMAGINATION PROXIA D (PERPETUAL VIEW GENERATION)
...
e.g On an ASTRAL MINDCLOUD this PROXIA can be used to process INFORMATION sent to it from an INSTINCTIVE MIND PROXIA/MINDCLOUD (OBJECT DETECTION). So for example if the ROBOT takes a picture of a location with its eye cameras (including from DRONE-BOT) or if it downloads a picture of a locstion from the internet (at a later time) it can IMAGINE/VISUALIZE what it might be like to fly through scene.
